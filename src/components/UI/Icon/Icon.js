...
const Icon = ({ type }) => {
  if (type === 'active') {
    return <span className={`${styles.dot} ${styles.active}`} data-testid="Icon-active"></span>;
  }
  if (type === 'inactive') {
    return <span className={`${styles.dot} ${styles.inactive}`} data-testid="Icon-inactive"></span>;
  }
  if (type === 'vehicle') {
